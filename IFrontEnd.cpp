...
private:
    IBackEnd* backEnd;
    vector<string> queriesIndex;
    vector<string> adsIndex;
    vector<entry> clickGraph; // clickgraph
...
public:
    ...
    IQueryResult* matchAd(std::string query, const IUser*
        ...
        this->backEnd->matchAdRewrites(queries, user, foundAd);
    };
    ...
    void simrank() {
        ...
        //        std::cout << "Result Output Complete:\n" << S << std::endl;
        //        for (int i = 0; i < queriesIndex.size(); i++) {
        //            std::cout<< queriesIndex.at(i) << " : " <<std::endl;
        //            for (int j = 0; j < queriesIndex.size(); j++) {
        //                std::cout<<"\t"<< queriesIndex.at(j)<< " = "<< S(i,j) << std::endl;
        //            }
        //        }
...
//                std::cout << "value: " << P(a, i) << "\n" << std::endl;
    ...
    double spread(boost::numeric::ublas::matrix_column<boost::numeric::ublas::matrix<double> > col) {
        ...
        vector<double> probability = calcProbability(col);
        // Erwartungswert
        ...
        for (unsigned i = 0; i < col.size(); ++i) {
            if (col(i) != 0) {
                count++;
                variance += pow((col(i) - expectation), 2) * probability.at(i);
...
//        std::cout << "spread: " << spread << std::endl;
    ...
    vector<double> calcProbability(boost::numeric::ublas::matrix_column<boost::numeric::ublas::matrix<double> > col) {
        vector<double> prob;
        double sum = 0;
        for (int i = 0; i < col.size(); i++) {
            sum += col(i);
